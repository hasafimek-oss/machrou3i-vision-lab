...
import { Menu, X, Globe, Sparkles, LogOut } from "lucide-react";
    ...
    >
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold font-heading text-gradient-neon">{t("brand")}</span>
        </Link>

        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link key={link.to} to={link.to}
              className={`text-sm font-medium transition-colors hover:text-neon ${isActive(link.to) ? "text-neon" : "text-muted-foreground"}`}
            >{link.label}</Link>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
                ...
                >
                  {languages.map((lang) => (
                    <button key={lang.code} onClick={() => switchLang(lang.code, lang.dir)}
                      className={`block w-full px-4 py-2 text-sm text-start hover:bg-primary/20 transition-colors ${i18n.language === lang.code ? "text-neon" : "text-muted-foreground"}`}
                    >{lang.label}</button>
                  ))}
                </motion.div>
              )}
          ...
          {user ? (
            ...
            <>
              ...
              <Link to="/signup" className="px-4 py-2 text-sm font-semibold rounded-lg bg-neon text-accent-foreground hover:opacity-90 transition-opacity glow-neon">{t("nav.signup")}</Link>
            </>
          )}
        </div>
              ...
              <div className="flex gap-2">
                {languages.map((lang) => (
                  <button key={lang.code} onClick={() => { switchLang(lang.code, lang.dir); setMobileOpen(false); }}
                    className={`px-3 py-1 text-xs rounded-md ${i18n.language === lang.code ? "bg-neon text-accent-foreground" : "glass-panel text-muted-foreground"}`}
                  >{lang.label}</button>
                ))}
              </div>
            </div>
